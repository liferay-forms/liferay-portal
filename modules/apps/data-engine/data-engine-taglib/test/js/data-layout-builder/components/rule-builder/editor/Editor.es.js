			...
			describe('Field operator', () => {
				it.each(STRING_DATATYPE_FIELDS)(
					...
					async (firstArg) => {
						...
						fireEvent.click(getByText(firstArg));

						expect(getByText('Is equal to'));
						expect(getByText('Is not equal to'));
						expect(getByText('Contains'));
						expect(getByText('Does not contain'));
						expect(getByText('Is empty'));
						expect(getByText('Is not empty'));
					}
				);
				...
				it.each(NUMBER_TYPE_FIELDS)(
					...
					async (firstArg) => {
						...
						expect(
							getByText('Is greater than or equal to')
						).toBeTruthy();
						expect(getByText('Is less than')).toBeTruthy();
						expect(getByText('Is not empty')).toBeTruthy();
						expect(getByText('Is empty')).toBeTruthy();
						expect(getByText('Is not empty')).toBeTruthy();
					}
				);
			});
			...
			describe('Binary operations', () => {
				it.each(
					...
					async (firstArg) => {
						...
						if (firstArg === 'text') {
							expect(
								document
									...
									.querySelector('input.ddm-field-text')
							).toBeTruthy();
						}
						else if (
							firstArg === 'select' ||
							firstArg == 'checkbox_multiple' ||
							firstArg == 'radio'
						) {
							expect(
								document
									.querySelectorAll('.timeline-item')[1]
									.querySelectorAll('.ddm-field')[3]
									.querySelector('.form-builder-select-field')
							).toBeTruthy();
						}
						else if (firstArg === 'date') {
							expect(
								document
									.querySelectorAll('.timeline-item')[1]
									.querySelectorAll('.ddm-field')[3]
							...
							expect(
								document
									.querySelectorAll('.timeline-item')[1]
									.querySelectorAll('.ddm-field')[3]
									.querySelector('.clay-color-picker')
							).toBeTruthy();
						}
						else if (firstArg === 'image') {
							expect(
								document
									.querySelectorAll('.timeline-item')[1]
									.querySelectorAll('.ddm-field')[3]
						...
						else if (firstArg === 'document_library') {
							expect(
								document
									.querySelectorAll('.timeline-item')[1]
									.querySelectorAll('.ddm-field')[3]
				...
				it('show the all others fields when action type is Other field', async () => {
					const props = defaultProps();
					const mockIsSignedIn = jest.fn();
			...
			it.each(['show', 'require', 'enable'])(
				...
				async (firstArg) => {
					...
					fields.forEach((field) => {
						const fieldOccurences = queryAllByText(field);
