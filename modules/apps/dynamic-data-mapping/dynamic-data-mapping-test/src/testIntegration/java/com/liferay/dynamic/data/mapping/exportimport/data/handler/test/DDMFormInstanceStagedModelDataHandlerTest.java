...
public class DDMFormInstanceStagedModelDataHandlerTest
	...
	public void testImportExportDDMFormInstanceWithObjectStorageType()
		...
		_assertExportImport(ddmFormInstance, objectDefinition);
	}
	...
	private void _assertDDMFormInstanceSettings(
			String settings, long expectedObjectDefinitionId,
			String expectedObjectDefinitionExternalReferenceCode)
		throws Exception {

		JSONArray fieldValuesJSONArray = _jsonFactory.createJSONObject(
			settings
		).getJSONArray(
			"fieldValues"
		);
	...
	private void _assertExportImport(
			StagedModel stagedModel, ObjectDefinition objectDefinition)
		throws Exception {
		...
		DDMFormInstance exportedDDMFormInstance =
			(DDMFormInstance)readExportedStagedModel(stagedModel);

		_assertDDMFormInstanceSettings(
			exportedDDMFormInstance.getSettings(),
		...
		StagedModelDataHandlerUtil.importStagedModel(
			portletDataContext, exportedDDMFormInstance);

		DDMFormInstance importedDDMFormInstance =
			(DDMFormInstance)getStagedModel(
				exportedDDMFormInstance.getUuid(), liveGroup);
