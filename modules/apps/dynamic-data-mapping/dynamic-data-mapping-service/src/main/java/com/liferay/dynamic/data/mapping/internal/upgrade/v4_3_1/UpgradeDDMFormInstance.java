...
public class UpgradeDDMFormInstance extends UpgradeProcess {
		...
		try (ResultSet rs = selectPreparedStatement.executeQuery()) {
			while (rs.next()) {
				...
				if (_upgradeSettings(settingsJSONObject)) {
					...
					long formInstanceId = rs.getLong(idColumnName);

					updatePreparedStatement.setLong(2, formInstanceId);
	...
	private boolean _upgradeSettings(JSONObject settingsJSONObject) {
		...
		for (int i = 0; i < fieldValuesJSONArray.length(); i++) {
			JSONObject settingJSONObject = fieldValuesJSONArray.getJSONObject(
				i);

			if (StringUtil.equals(
					settingJSONObject.getString("name"),
					"workflowDefinition")) {

				String workflowDefinition = settingJSONObject.getString(
					"value");
