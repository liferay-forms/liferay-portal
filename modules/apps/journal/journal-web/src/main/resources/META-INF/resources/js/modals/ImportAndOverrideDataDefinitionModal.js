...
const ImportAndOverrideDataDefinitionModal = ({portletNamespace}) => {
	...
	useEffect(() => {
		const openImportAndOverrideFormModal = ({
			importAndOverrideDDMStructureURL,
		}) => {
			setImportAndOverrideStructureURL(importAndOverrideDDMStructureURL);
			setVisible(true);
		};

		Liferay.on(
			'openImportAndOverrideFormModal',
			openImportAndOverrideFormModal
		);

		return () => {
			Liferay.detach('openImportAndOverrideFormModal');
		};
	}, []);
